...
function wireNavToggle() {
  ...
  if (window.__hoganMobileDrawerBound) {
    if (debugNav) console.log("[nav]", "drawer binding skipped from site.js", window.__hoganMobileDrawerBound);
    return;
  }
  const toggles = Array.from(document.querySelectorAll('.navtoggle[aria-controls="site-nav"], .topbar__icon--menu'));
  if (!toggles.length) return;
  window.__hoganMobileDrawerBound = "site.js";
  ...
  const ensureDrawer = () => {
    ...
    if (debugNav) {
      console.log("[nav]", "drawer ready from site.js", {
  ...
  const openDrawer = () => {
    ...
    syncButtons(true);
    if (debugNav) {
      console.log("[nav]", "drawer opened from site.js", {
        hidden: drawer.hidden,
        className: drawer.className,
      });
    }
  ...
  const closeDrawer = (reason = "close") => {
    ...
    syncButtons(false);
    if (debugNav) {
      console.log("[nav]", "drawer closed from site.js", {
        reason,
        hidden: drawer.hidden,
        className: drawer.className,
      });
  ...
  if (debugNav) console.log("[nav]", "drawer binding from site.js");
